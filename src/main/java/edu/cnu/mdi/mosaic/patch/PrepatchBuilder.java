 ...
 * <p>
 * This first implementation handles
 * {@link CellIntersectionType#CORNER_STRADDLE} cells. Other intersecting-cell
 * types are recorded as deferred rather than guessed, so the algorithm remains
...
public final class PrepatchBuilder {
	 ...
	 */
	public static PrepatchBuildResult buildPrepatches(List<IntersectingCell> cells, double radius) {

		if (cells == null) {
			return new PrepatchBuildResult(List.of(), List.of(), List.of(), PrepatchBuildStats.empty());
		}
		...
		EnumMap<PrepatchFailureReason, Integer> failureCounts = new EnumMap<>(PrepatchFailureReason.class);
		...
		for (IntersectingCell cell : cells) {
			...
			case CORNER_STRADDLE -> {
				...
				if (result.prepatch != null && result.prepatch.hasCurves()) {
					...
					if (failure == null) {
						failure = new PrepatchFailure(cell.id(), PrepatchFailureReason.TOO_FEW_CURVES, 0, new int[6],
								"Ordinary prepatch construction failed without a specific reason.");
					}

					failures.add(failure);
					failureCounts.put(failure.reason(), failureCounts.get(failure.reason()) + 1);
				}
			}
		...
		PrepatchBuildStats stats = new PrepatchBuildStats(ordinaryVisited, prepatches.size(), curvesBuilt,
				deferredFacePenetration, deferredBoundary, deferredTangent, failures.size(), failureCounts);
	 ...
	 */
	private static OrdinaryBuildResult buildOrdinaryPrepatch(IntersectingCell cell, double radius) {

		CellGeometry geometry = cell.geometry();

		List<SphereEdgeIntersection> intersections = findEdgeIntersections(geometry, radius);

		int[] faceHitCounts = faceHitCounts(intersections);

		if (intersections.size() < 3) {
			return OrdinaryBuildResult
					.failure(new PrepatchFailure(cell.id(), PrepatchFailureReason.TOO_FEW_EDGE_INTERSECTIONS,
							intersections.size(), faceHitCounts, "Expected at least 3 edge-sphere intersections."));
		}

		ArrayList<GeneralCurve> curves = new ArrayList<>();

		for (int face = 0; face < 6; face++) {
			List<SphereEdgeIntersection> faceHits = intersectionsOnFace(intersections, face);
			...
			if (faceHits.size() != 2) {
				return OrdinaryBuildResult.failure(new PrepatchFailure(cell.id(),
						PrepatchFailureReason.FACE_HIT_COUNT_NOT_TWO, intersections.size(), faceHitCounts,
						"Face " + face + " has " + faceHits.size() + " hits; ordinary face arc requires exactly 2."));
			}

			GeneralCurve curve = GeneralCurve.create(geometry, face, radius, faceHits.get(0).point(),
					faceHits.get(1).point());

			if (curve == null) {
				return OrdinaryBuildResult.failure(new PrepatchFailure(cell.id(),
						PrepatchFailureReason.CURVE_CREATION_FAILED, intersections.size(), faceHitCounts,
		...
		if (curves.size() < 3) {
			return OrdinaryBuildResult.failure(new PrepatchFailure(cell.id(), PrepatchFailureReason.TOO_FEW_CURVES,
					intersections.size(), faceHitCounts,
					"Built only " + curves.size() + " curves; ordinary prepatch needs at least 3."));
		}

		PoleClassification poleClassification = PoleClassifier.classify(geometry, radius);

		return OrdinaryBuildResult.success(new Prepatch(cell.id(), cell.type(), curves, poleClassification));
	}
	 ...
	 */
	private static List<SphereEdgeIntersection> findEdgeIntersections(CellGeometry geometry, double radius) {
		...
		for (int edge = 0; edge < 12; edge++) {
			...
			if (p != null) {
				intersections
						.add(new SphereEdgeIntersection(edge, corners[0], corners[1], p, adjacentFacesForEdge(edge)));
			}
		}
	...
	private static Vec3 segmentSphereIntersection(Vec3 p0, Vec3 p1, double radius) {
		...
		return new Vec3(p0.x() + t * dx, p0.y() + t * dy, p0.z() + t * dz);
	}
	 ...
	 */
	private static List<SphereEdgeIntersection> intersectionsOnFace(List<SphereEdgeIntersection> intersections,
			int face) {
	...
	private static int[] adjacentFacesForEdge(int edge) {
		...
		if (faces.size() != 2) {
			throw new IllegalStateException("Canonical edge " + edge + " does not have two adjacent faces.");
		}
